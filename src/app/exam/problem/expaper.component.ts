	...
	loadPaper(x, y): void {
		...
		paperImg.onload = function() {
			if (t.isBigSize === false) {
				t.scale = paperImg.width/t.ctx.canvas.width;
				...
				t.ctx.canvas.height = (t.ctx.canvas.width/(paperImg.width)) * (paperImg.height) ;
				t.ctx.drawImage(paperImg, x*t.scale , y*t.scale, paperImg.width, paperImg.height, 0, 0, paperImg.width, paperImg.height);
			}
		}
